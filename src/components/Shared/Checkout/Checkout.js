...
const Checkout = () => {
  ...
  useEffect(() => {
    fetch('https://obscure-peak-91522.herokuapp.com/services')
      .then((res) => res.json())
  ...
  return (
    <div className='row'>
      ...
      <div
        style={{ display: order ? 'none' : 'block' }}
        className='col-md-8 p-5'
      >
        <h3 className='pb-2 text-info'>Checkout</h3>
        <hr />
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {!clickedService[0] ? (
            <div>
              <p style={{ color: 'red' }}>
                <FontAwesomeIcon icon={faExclamationCircle} /> Please select a
                service.
      ...
      <div
        ...
        className='col-md-5 p-2 m-2'
      >
        <h3 className='text-primary'>
          Payment <small>(Pay with Stripe)</small>
        </h3>{' '}
        <hr />
